...
from strategies.supertrend_luxalgo_strategy import (
    SupertrendRangeOscStrategy,
    SupertrendTrendlineStrategy
)
...
def run_luxalgo_test(symbol='SOL', initial_equity=10000.0):
    """
    运行LuxAlgo指标策略测试
    ...
    """
    print("="*80)
    print(f"SUPERTREND + LUXALGO STRATEGIES TEST - {symbol}")
    print("="*80)
    ...
    strategies = [
        ...
        (SupertrendRangeOscStrategy(atr_period=10, factor=3.0, ro_length=50, ro_mult=2.0),
         "RangeOsc_Filter",
         "Range Oscillator过滤（暂停Range市场）"),

        (SupertrendTrendlineStrategy(atr_period=10, factor=3.0, tl_length=14, tl_mult=1.0),
         "Trendline_Confirm",
         "Trendlines突破确认"),
    ]
    ...
    for strategy, strategy_name, description in strategies:
        ...
        # 如果是RangeOsc策略，计算Range Oscillator
        if 'RangeOsc' in strategy_name:
            df_test = strategy.calculate_range_oscillator(df_test)

        # 如果是Trendline策略，计算Trendlines
        if 'Trendline' in strategy_name:
            df_test = strategy.calculate_trendlines(df_test)
        ...
        # 计算信号统计
        if 'RangeOsc' in strategy_name:
            range_filtered = sum(1 for e in entries if e.get('ro_in_range', False))
            print(f"\nRange Oscillator Statistics:")
            print(f"  Range Market Signals: {range_filtered}/{len(entries_list)} (should be 0)")

        if 'Trendline' in strategy_name:
            break_up = sum(1 for e in entries if e.get('tl_break_up', False))
            break_down = sum(1 for e in entries if e.get('tl_break_down', False))
            print(f"\nTrendlines Statistics:")
            print(f"  Upward Breakouts: {break_up}")
            print(f"  Downward Breakouts: {break_down}")
...
def main():
    print("\n" + "█"*80)
    print("█" + " "*78 + "█")
    print("█" + " "*15 + "SUPERTREND + LUXALGO STRATEGIES TEST" + " "*27 + "█")
    print("█" + " "*78 + "█")
    print("█"*80 + "\n")

    # 测试SOL
    print("\n📊 Testing SOL (Best performing symbol for Supertrend)")
    sol_results = run_luxalgo_test('SOL')

    # 测试BNB
    print("\n\n📊 Testing BNB")
    bnb_results = run_luxalgo_test('BNB')
    ...
    if sol_improved or bnb_improved:
        print("✅ LuxAlgo filters show improvement on some symbols!")
        if sol_improved:
        ...
        print("⚠️  LuxAlgo filters did NOT improve performance.")
